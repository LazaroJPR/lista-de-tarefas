...
class TarefaController(
    ...
    ): ResponseEntity<Tarefa> = ResponseEntity.ok(tarefaService.salvarTarefa(tarefa))

    @PutMapping("/{id}")
    fun atualizarTarefa(
        @PathVariable id: Long,
        @RequestBody tarefa: Tarefa,
    ): ResponseEntity<Tarefa> = ResponseEntity.ok(tarefaService.atualizarTarefa(id, tarefa))
